...
SOCKADDR_IN GetRequestFromClient(char* name, short port, unsigned short& clntport)
{
	...
	char servname[10] = { *name, *(name + 1), *(name + 2), *(name + 3), *(name + 4), *(name + 5), *(name + 6), *(name + 7), *(name + 8), '\0' };
	...
	while (true)
	{
		...
		{
			string v1 = inet_ntoa(clnt.sin_addr);
			hostent* clntHostent = gethostbyaddr((char*)&(clnt.sin_addr), sizeof(inet_ntoa(clnt.sin_addr)), AF_INET);
			cout << "Коннект есть" << endl;
			cout << "Символьное имя клиента: " << clntHostent->h_name << endl;
...
int main()
{
	...
	try
	{
		...
		cout << "Имя системы:\t" << hostname << endl << endl;
		...
		do
		{
			...
			//Отправка имени сервера клиенту
			SOCKADDR_IN to;
