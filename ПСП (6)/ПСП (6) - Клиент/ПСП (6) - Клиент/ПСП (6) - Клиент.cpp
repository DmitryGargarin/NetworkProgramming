...
int main()
{
	...
	try
	{
		...
		char bto[50], bfrom[50], msgIn[40] = "Hello from Client - ", clnt[10] = "Client: \0", stop[10] = "\0";

		for (int i = 0; i < count; i++)
		{
			pipe = CreateFile(L"\\\\.\\pipe\\Kostas", GENERIC_WRITE | GENERIC_READ, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
			if (pipe == INVALID_HANDLE_VALUE)
				throw SetErrorMsgText("CreateFile", GetLastError());
			char numb[5];
			const char* temp = itoa(i, numb, 10);
			strcpy(bto, clnt); strcat_s(bto, temp);
			if (!WriteFile(pipe, bto, sizeof(bto), &written, NULL))
		...
		if (pipe == INVALID_HANDLE_VALUE)
			throw SetErrorMsgText("CreateFile", GetLastError());
		if (!WriteFile(pipe, stop, sizeof(stop), &written, NULL))
			throw SetErrorMsgText("WriteFile", GetLastError());
	}
